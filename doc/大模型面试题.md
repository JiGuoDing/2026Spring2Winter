...
这个问题的核心在于区分“模型学会了什么”和“系统替模型做了什么”。严格来说，LLM 并没有在参数里真的学会执行 API，它学会的是三件事：识别何时该用工具、选择哪个工具、生成正确参数。真正的执行仍由外部运行时完成。

面试时可以先给一句总览：LLM 的工具调用能力通常来自“训练对齐 + 推理约束 + 运行时反馈”三层机制协同，而不是单一技巧。

#### 一、训练层：模型如何形成工具调用习惯

##### 1. 预训练阶段 (Pretraining)

预训练数据里包含大量代码、API 文档、函数签名和调用示例，模型会先学到“函数调用是一种常见文本模式”。这一步还不是可靠的工具调用能力，更像是语法与模式记忆。

##### 2. 指令微调阶段 (SFT)

真正关键的是带标注的工具调用样本，常见样本形态是：

- 用户问题
- 可用工具定义 (name/description/schema)
- 期望输出 (调用某工具及参数 JSON)

模型通过监督学习拟合“在什么意图下输出什么 tool call”。本质是条件生成问题：

$$
\max\_{\theta}\;\sum\_{(x, y)} \log P\_{\theta}(y\mid x)
$$

其中 $x$ 包含用户问题和工具描述，$y$ 是目标调用结构。

##### 3. 偏好对齐阶段 (RLHF/DPO)

仅有 SFT 还不够，模型可能会“该调不调”或“乱调工具”。偏好对齐会进一步优化以下行为：

- 在可直接回答时避免多余调用
- 在信息不足时优先发起工具查询
- 多工具可选时偏向更准确或成本更低的工具

因此，工具调用能力不是一个二元能力，而是一个“决策质量”持续优化的过程。

#### 二、推理层：为什么模型能稳定产出可执行调用

很多人以为模型“天生就能输出 JSON”，其实稳定性主要来自推理时约束：

- 把工具 schema 注入上下文，明确参数类型、必填项、枚举范围
- 使用 constrained decoding 或 grammar-based decoding，限制输出空间
- 配合 structured output parser 做语法和字段校验

可以把它理解为：训练决定“会不会”，约束决定“稳不稳”。
...
用户请求
    ↓
LLM 决策 (是否调用工具/调用哪个工具)
    ↓
生成结构化参数
    ↓
Runtime 校验 + 执行工具
    ↓
返回 Observation (工具结果/报错信息)
    ↓
LLM 二次决策 (继续调用/总结回答)
```

如果执行失败，系统会把错误信息反馈给模型，例如“缺失 city 字段”或“日期格式非法”，模型据此修正参数再调用。这个机制本质上是测试时的自纠错闭环，能显著提升成功率。
...
question = "帮我看看订单 A20260417001 到哪了"

# step1: LLM 决定调用 query_order, 并生成 arguments
tool_call = llm.decide_and_format_call(question, tools)

# step2: Runtime 做参数校验
ok, err = validate(tool_call["arguments"], tools[0]["parameters"])
if not ok:
        # 把错误反馈给 LLM 进行参数修正
        tool_call = llm.repair_call(question, tool_call, err)

# step3: 执行工具并返回结果
obs = query_order(**tool_call["arguments"])

# step4: LLM 基于 observation 给出最终自然语言答复
answer = llm.summarize(question, obs)
```

这段代码体现了三个关键能力：意图识别、参数构造、错误修复。面试官通常会看你是否意识到“校验和重试属于运行时职责，而不是模型本体职责”。

#### 五、工程上常见的能力增强手段

- 工具检索 (Tool Retrieval)：当工具很多时，先检索候选工具再给模型，降低选择噪声
- 参数补全策略：对缺失参数做多轮追问，而不是盲猜
- 失败重试策略：按错误类型重试，不做无意义重复调用
- 工具调用预算：限制调用次数、并发度和超时，防止代理失控
- 轨迹蒸馏 (Trajectory Distillation)：把高质量调用轨迹回流为训练样本，持续提升模型

#### 六、边界与常见误区

##### 1. 误区：只要给工具描述，模型就一定会正确调用

不成立。若描述模糊、参数命名混乱、schema 设计差，模型会显著退化。

##### 2. 误区：工具调用能力完全是 prompt engineering 的结果

不准确。Prompt 很重要，但上限主要由训练数据质量、对齐策略和运行时约束共同决定。

##### 3. 误区：工具越多越强

不成立。工具集合过大且同质化时，模型选择不确定性上升，反而更容易选错。

#### 七、面试时可以怎么总结

可以这样回答：LLM 学会调用外部工具，本质上不是学会“执行函数”，而是学会“工具调用决策与参数生成”。它先通过预训练和 SFT 学会工具调用模式，再通过偏好对齐优化调用决策质量，在推理时借助 schema 约束和结构化解码保证输出可执行，最后通过运行时校验和错误反馈形成闭环纠错。因此工具调用能力是模型能力与系统工程共同产物，不是单点技巧。

#### 知识扩展

- ReAct：工具调用学习可看作 ReAct 中 Action 策略学习，Observation 反馈会反向提升下一步决策质量。
- Agent Planning：当任务是多步子目标时，工具调用学习要和规划能力联合优化。
- Toolformer：代表性研究方向，核心思想是让模型在训练中学习“何时调用 API”。
- Structured Decoding：与函数调用强相关，决定结构化输出的可解析率和稳定性。
- Offline Eval 与 Online Eval：前者评估调用正确率，后者评估真实环境下成功率与成本，二者共同决定可用性。
