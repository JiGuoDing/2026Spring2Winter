...
- M: 机器线程 (OS Thread)，真实的操作系统县城，负责执行 Go 代码，M 必须拿到一个 P 才能执行 Go 代码 (没有 P 的 M 只能阻塞/空转，或去做系统调用相关的事情)
- P: 处理器 (调度资源/执行上下文)，调度器的关键资源，决定 "并行度"，持有本地运行队列 (run queue)、缓存等。P 的数量 = GOMAXPROCS (默认等于 CPU 核数)，表示同一时刻最多有多少个 goroutine 真正在 CPU 上并行跑 (不含阻塞在 syscall 的线程)
