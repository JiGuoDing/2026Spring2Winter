...
### 1.8 ClickHouse 一次 SQL 查询的典型执行流程是怎样的？

ClickHouse 一次 SQL 查询的执行过程，可以理解为从客户端提交 SQL 开始，经过解析、分析、优化、读取数据、执行算子、合并结果，最后返回客户端的完整链路。

面试里可以先用一句话概括：ClickHouse 查询执行的核心流程是先把 SQL 转换成执行计划，再根据分区、主键索引和数据跳数索引尽量减少扫描数据，最后通过向量化和多线程执行过滤、聚合、Join、排序等算子。

#### 一、客户端提交 SQL

客户端可以通过 clickhouse-client、HTTP、JDBC、ODBC 或 Native TCP 协议连接 ClickHouse Server。

用户提交 SQL 后，请求会先到达某一个 ClickHouse Server。这个节点可能只执行本地查询，也可能作为分布式查询的发起节点，把查询下发到其他分片。

#### 二、SQL 解析和语义分析

ClickHouse 收到 SQL 后，会先进行语法解析，把 SQL 转换成内部语法树。
...
2. 校验数据类型是否匹配。
3. 展开表达式、别名和函数调用。
4. 判断查询涉及的是本地表还是 Distributed 表。

这一步的目标是把用户写的 SQL 转换成 ClickHouse 可以理解和执行的内部表示。

#### 三、查询优化和执行计划生成

语义分析完成后，ClickHouse 会生成查询计划，并进行一些优化。

常见优化包括：

1. 谓词下推：尽量把过滤条件下推到数据读取阶段。
2. 列裁剪：只读取查询真正需要的列。
3. 分区裁剪：根据 PARTITION BY 跳过无关分区。
4. 主键索引裁剪：根据 ORDER BY 和 Primary Key 定位可能命中的 Granule。
5. 数据跳数索引裁剪：利用 BloomFilter、MinMax、Set 等索引跳过无关数据块。

这些优化的核心目标是减少后续真正需要读取和计算的数据量。

#### 四、读取 MergeTree 数据

如果查询的是 MergeTree 系列表，ClickHouse 会根据执行计划读取对应的 Part、列文件和 Granule。

读取数据时通常会经历：

1. 根据分区条件选择需要扫描的 Partition。
2. 根据主键稀疏索引定位需要读取的 Mark Range。
3. 根据数据跳数索引进一步跳过不相关数据块。
4. 只读取查询需要的列，而不是整行数据。
5. 解压列数据，并按 Block 形式交给后续算子处理。

这里的关键是 ClickHouse 尽量避免全量扫描无关数据。

#### 五、执行查询算子

数据读取出来后，ClickHouse 会以列式 Block 为单位执行查询算子。

常见算子包括：

- Filter：执行 WHERE 条件过滤。
- Expression：计算表达式、函数和类型转换。
- Aggregate：执行 GROUP BY 和聚合函数。
- Join：构建和探测 Hash Table，完成表连接。
- Sort：执行 ORDER BY 排序。
- Limit：限制返回行数。

ClickHouse 会尽量使用向量化执行和多线程并行处理数据，提高 CPU 利用率。

#### 六、分布式查询的执行流程

如果查询的是 Distributed 表，执行链路会多一步分布式下发和结果汇总。

典型流程是：

1. 客户端连接一个发起节点。
2. 发起节点根据集群配置找到相关 Shard。
3. 发起节点把子查询下发到各个分片的本地表。
4. 各个分片并行执行本地查询。
5. 分片返回局部结果。
6. 发起节点进行最终聚合、排序、Limit 或结果合并。
7. 最终结果返回客户端。

如果查询可以在各个分片先做局部聚合，再在发起节点做最终聚合，整体效率通常会更好。

#### 七、返回结果

执行完成后，ClickHouse 会把结果按照客户端协议返回。对于大结果集，结果通常会分批发送，而不是一次性全部放入内存。

如果查询包含 ORDER BY、GROUP BY、DISTINCT 或 Join，可能会消耗较多内存，因此需要关注 max_memory_usage、max_threads、max_bytes_before_external_group_by、max_bytes_before_external_sort 等参数。

#### 八、面试时可以怎么总结

可以这样回答：ClickHouse 一次查询通常从客户端提交 SQL 开始，Server 先进行解析和语义分析，然后生成执行计划并做优化，比如列裁剪、谓词下推、分区裁剪、主键索引裁剪和数据跳数索引裁剪。真正读取 MergeTree 数据时，会按 Part、Mark Range 和列文件读取尽量少的数据，再以列式 Block 为单位执行过滤、表达式计算、聚合、Join、排序等算子。如果查询的是 Distributed 表，发起节点会把子查询下发到多个分片并行执行，最后汇总结果返回客户端。整体上，ClickHouse 查询快的关键是少读数据、并行处理、向量化执行和尽量提前做数据裁剪。

#### 知识扩展

- Query Plan：SQL 被转换成可执行计划的中间表示
- Predicate Pushdown：把过滤条件尽量下推到扫描阶段
- Column Pruning：只读取查询需要的列
- Mark Range：ClickHouse 读取 MergeTree 数据的重要范围单位
- Block：ClickHouse 向量化执行时处理数据的基本批次
