...
建议把 Task State 单独建模，而不是埋在自然语言摘要里，形成一个 Scratchpad 记录任务状态。例如：
...
Skill 把某一类任务的执行经验固化为可复用的模块。对于经过设计、评测和版本治理的 Skill，调用方可以复用既定流程与质量约束，减少重复组织上下文和推理路径，并提升输出的一致性、可控性和任务成功率。
